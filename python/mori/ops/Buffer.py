...
from mori.ops.dispatch_combine_deepep import (
...
class Buffer:
    ...
    def __init__(self, group: dist.ProcessGroup,
        ...
        self.config = None
        self.deepep_ops = None
        self.deepep_config = None
    ...
    def low_latency_dispatch(self, x: torch.Tensor, topk_idx: torch.Tensor,
        ...
        if self._should_use_deepep_ll():
            ...
            topk_idx_ll = topk_idx.to(dtype=torch.int32)
            deepep_op = self._get_deepep_op(
                x.dtype,
                x.size(1),
                scale_dim,
                use_fp8,
            ...
            recv_x, recv_count, ll_handle, _, _ = deepep_op.dispatch_deepep_ll(
                x,
                topk_idx_ll,
            ...
            dispatch_weights, dispatch_indices = ll_handle
    ...
    def low_latency_combine(self, x: torch.Tensor, topk_idx: torch.Tensor, topk_weights: torch.Tensor,
        ...
        if self._should_use_deepep_ll() and handle and len(handle) >= 4 and handle[0] == "deepep_ll":
            if len(handle) >= 5:
                ...
                deepep_op = self.deepep_ops
                if deepep_op is None:
                    raise RuntimeError("DeepEP LL handle missing op; run low_latency_dispatch first.")
                _, dispatch_indices, dispatch_weights, use_fp8 = handle
            combine_output, _, _ = deepep_op.combine_deepep_ll(
                x,
                dispatch_indices,
                dispatch_weights,
                block_num=self.config.block_num if self.config is not None else -1,
                warp_per_block=16,
            )
